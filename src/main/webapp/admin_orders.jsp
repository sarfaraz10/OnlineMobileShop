		...
		while(orderResultSet.next()){
			...
			<%
				if(status.equals("waiting")){
					%>
						<button style="padding: 15px;margin-right:30px; text-align: center;" value="clicked" class="roundGreen" onclick="window.location.href = 'admin_approve_order.jsp?orderid=<%=orderId%>';">Approve</button>
						<button style="padding: 15px; text-align: center;" value="clicked" class="roundRed" onclick="window.location.href = 'admin_reject_order.jsp?orderid=<%=orderId%>';">Reject</button>
					<%
				}
				else if(status.equals("approved")){
					%>
					<button style="padding: 15px;margin-right:30px; text-align: center;" value="clicked" class="roundGreen" onclick="window.location.href = 'admin_deliver_order.jsp?orderid=<%=orderId%>';">Deliver</button>
					<button style="padding: 15px; text-align: center;" value="clicked" class="roundRed" onclick="window.location.href = 'admin_reject_order.jsp?orderid=<%=orderId%>';">Reject</button>
					<%
				}
				else if(status.equals("delivered")){
					out.println("<Label style='color:#0FA400;font-size:20px'>This product has delivered</Label>");
				}
				else{
					out.println("<Label style='color:red;font-size:20px'>This product has Rejected by admin</Label>");
				}
			%>
			</div>
			<%
